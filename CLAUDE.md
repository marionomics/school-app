...
- **⚠️ PostgreSQL FK cascade ordering**: SQLite ignores FK constraints by default; PostgreSQL enforces them strictly. When deleting a row that cascades to child rows, any grandchild FK references to those children must be cleaned up *first* — PostgreSQL will block the child delete otherwise. Pattern: before `db.delete(parent)`, bulk-delete grandchildren that reference the children, then `db.flush()`, then proceed. Example in this codebase: `ForumPost` cascade-deletes `ForumLike`; `ForumPoints.like_id` FK references `ForumLike`; so `forum_points` rows with `like_id` must be deleted (bulk + flush) before the post is deleted. Bugs of this type work fine in local dev (SQLite) but fail in production (PostgreSQL) with `ForeignKeyViolation`.
